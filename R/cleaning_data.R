...
#' Function that cleans the department codes of the disease data
#' @param disease_data The disease data
#' @param col_data_codes Column name in the disease data that contains the department codes
#' @param geo_codes Geographical data that includes the departments codes
#' @param col_geo_codes Column name in the geographical data than contains the department codes
#' @return Clean department codes of the disease data
#' @examples
#' geo_codes <- import_geo_codes()
#' disease_data <- import_data_disease_by_year(2019, "DENGUE")
...
#' @param col_uni_met Column name in the disease data that contains the measurement units
#' @return The ages in years
#' @examples
#' disease_data <- import_data_disease_by_year(2019, "DENGUE")
#' parse_age_to_years(disease_data, col_age = "edad", col_uni_met = "uni_med")
#' @export
parse_age_to_years <- function(disease_data, col_age = "edad", col_uni_met = "uni_med") {
  disease_dt_to_years <- dplyr::mutate(disease_data,
    edad = dplyr::case_when(
      eval(parse(text = col_uni_met)) == 1 ~ round(eval(parse(text = col_age)), 3),
      eval(parse(text = col_uni_met)) == 2 ~ round((eval(parse(text = col_age)) / 12), 3),
      eval(parse(text = col_uni_met)) == 3 ~ round((eval(parse(text = col_age)) / 876), 3),
      eval(parse(text = col_uni_met)) == 4 ~ round((eval(parse(text = col_age)) / 525960), 3),
      eval(parse(text = col_uni_met)) == 5 ~ round((eval(parse(text = col_age)) / 3.156e+7), 3)
    )
  )
...
#'
#' Function that removes a rows if the selected column values includes NA, Infinitive or NaN
#' @param disease_data The disease data
...
cleansing_sivigila_data <- function(disease_data, year) {
  ...
  clean_disease_data <- clean_depto_codes(clean_disease_data,
    col_data_codes = depto_column_names[1],
    geo_data = geo_country_data, col_geo_codes = "c_digo_departamento"
  )

  clean_disease_data <- parse_age_to_years(clean_disease_data, col_age = "edad", col_uni_met = "uni_med")
}