...
get_path_data_disease_year <- function(year, disease_name) {
  ...
  query_disease_path <- paste(base_path, paste(
    microdata_path, 
    query_path, 
    sep = ""), 
    sep = "")
  ...
  content_type_response <- stringr::str_split_fixed(
    httr::headers(get_query_disease)$`content-type`, 
    pattern = ";", 3)
  ...
  edit_node_query_disease_content <- xml2::xml_child(
    entry_nodes_query_disease_content, 
    12)
  ...
  content_type_response <- stringr::str_split_fixed(
    httr::headers(get_info_file)$`content-type`, 
    pattern = ";",
    3)
  content_type_response <- stringr::str_replace(
                             content_type_response[[1]], 
