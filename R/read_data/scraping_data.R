...
get_path_data_disease_by_year <- function(year, disease_name) {
  ...
  get_query_disease <- httr::GET(query_disease_path, xml2::add_headers("Accept" = "*/*"))
  
  content_type_response <- stringr::str_split_fixed(xml2::headers(get_query_disease)$`content-type`, pattern = ";", 3)
  content_type_response <- stringr::str_replace(content_type_response[[1]], "atom\\+", "")
  query_disease_content <- xml2::content(get_query_disease, type = content_type_response, encoding = "UTF-8")
  ...
  get_info_file <- httr::GET(paste(base_path, list_file_path,  sep = ""), xml2::add_headers("Accept" = "*/*"))
  
  content_type_response <- stringr::str_split_fixed(xml2::headers(get_info_file)$`content-type`, pattern = ";", 3)
  content_type_response <- stringr::str_replace(content_type_response[[1]], "atom\\+", "")
  info_file_content <- xml2::content(get_info_file, type = content_type_response, encoding = "UTF-8")
