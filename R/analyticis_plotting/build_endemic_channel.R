...
get_disease_years <- function(year, disease_name) {
  years_endemic_chanel <- c((year - 4):year)
  
  disease_dt_by_week <- import_data_disease_by_year((year - 5), disease_name)  %>% group_by_variable("SEMANA")
  names(disease_dt_by_week)[names(disease_dt_by_week) == "Casos"] <- as.character((year - 5))
  
  for (yr in years_endemic_chanel) {
        disease_dt_by_week_and_year <- import_data_disease_by_year(yr, disease_name) %>% group_by_variable("SEMANA")
