...
- year:
  ...
  contiene el año de los datos de una enfermedad o evento.

- format_fecha:

  Un \`character\` (cadena de caracteres) que contiene el formato
  deseado de fecha; su valor por defecto es "%AAAA-%MM-%DD".

- col_fecha:

  Un \`character\` (cadena de caracteres) que contiene el nombre de la
  columna con la fecha que se desea limpiar en los datos de la
  enfermedad o evento.

- col_comp:

  Un \`character\` (cadena de caracteres) que contiene el nombre de la
  columna con la cual se va a comparar la columna \`col_fecha\` para
  limpiarla, estandarizarla o aplicar las reglas definidas.
