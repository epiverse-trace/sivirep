...
data_map_disease_deptos <- clean_depto_disease_codes(deptos_data,
                                                     filtered_data)
...
disease_dt_by_onset_sym$ini_sin <- as.Date(disease_data$ini_sin,
                                           format = "%AAAA-%MM-%DD")
disease_dt_by_onset_sym <- remove_nin_values(disease_data,
                                             "ini_sin")
disease_dt_by_onset_sym <- remove_error_dates(disease_dt_by_onset_sym)
disease_dt_by_onset_sym <- group_columns_cases(disease_dt_by_onset_sym,
                                         "ini_sin")
disease_dt_by_onset_sym[
  order(disease_dt_by_onset_sym$ini_sin, decreasing = TRUE), ]

plot_variable(disease_dt_by_onset_sym, var_x = "ini_sin",
              var_y = "casos",
              var_fill = "ini_sin",
              label_x = "Fecha de inicio de síntomas",
              label_y = "casos",
              diagram_title =
              "Diagrama para la variable Fecha de inicio de síntomas",
              legend_pos = "right",
              text_sz = 4,
              show_val = FALSE)
```
...
disease_data_dd <- group_columns_cases(disease_data_dd, "fec_def")
...
disease_data_not$fec_not <- as.Date(disease_data$fec_not,
                                    format = "%AAAA-%MM-%DD")
disease_data_not <- group_columns_cases(disease_data, "fec_not")
...
disease_dt_by_gender <- group_columns_cases(disease_data,
                                            "sexo",
                                            wt_percentage = TRUE)
...
disease_dt_by_gender_wk <- group_columns_cases(disease_data,
                                               c("semana", "sexo"),
                                               TRUE)
...
disease_dt_by_age <- group_columns_cases(disease_dt_by_age,
                                         c("edad", "semana"),
                                         wt_percentage = TRUE)
disease_dt_by_age_range <- group_age_range_cases(disease_dt_by_age,
                                                 "edad",
                                                 min_val = 0,
                                                 max_val =
                                  max(disease_dt_by_age$edad), step = 10)
...
disease_dt_by_age_gen <- group_columns_cases(disease_dt_by_age_gen,
                                             c("edad", "sexo"),
                                             wt_percentage = TRUE)
disease_dt_by_age_gen_range <- group_age_range_cases(disease_dt_by_age_gen,
                                                     "edad",
                                                     "sexo",
                                                     min_val = 0,
                                                     max_val =
                                        max(disease_dt_by_age_gen$edad), 10)
