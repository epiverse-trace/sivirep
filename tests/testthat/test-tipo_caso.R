...
test_that("`tipo_caso` maneja errores correctamente", {
  ...
  expect_error(agrupar_tipo_caso(data_event = list(a = 1, b = 2)),
               "El parametro data_event debe ser un data.frame")
  
  expect_error(agrupar_tipo_caso(data_event = data_limpia,
                                 cols_tipo = TRUE),
               "El parametro cols_tipo debe ser una cadena de caracteres")
  
  expect_error(plot_tipo_caso(data_agrupada = list(a = 1, b = 2)),
               "El parametro data_agrupada debe ser un data.frame")
  
  expect_error(plot_tipo_caso(data_agrupada = data_limpia, col_tipo = 1),
...
test_that("`tipo_caso_years` maneja errores correctamente", {
  ...
  expect_error(plot_tipo_caso_years(data_agrupada = list(a = 1, b = 2)),
               "El parametro data_agrupada debe ser un data.frame")
  
  expect_error(plot_tipo_caso_years(data_agrupada = data_limpia,
                                    col_tipo = 1),
               "El parametro col_tipo debe ser una cadena de caracteres")
  expect_error(plot_tipo_caso_years(data_agrupada = data_limpia,
