...
test_that("`mpio` maneja errores correctamente", {
  ...
  expect_error(agrupar_mpio(data_event = list(a = 1, b = 2)),
               "El parametro data_event debe ser un data.frame")
  
  expect_error(agrupar_mpio(data_event = data_limpia, col_mpio = TRUE),
               "El parametro col_mpio debe ser una cadena de caracteres")
  
  expect_error(agrupar_mpio(data_event = data_limpia,
                            porcentaje = "boolean"),
               "El parametro porcentaje debe ser un booleano")
  
  expect_error(plot_mpios(data_agrupada = list(a = 1, b = 2)),
               "El parametro data_agrupada debe ser un data.frame")
  
  expect_error(plot_mpios(data_agrupada = data_limpia,
                          col_mpios = 1),
