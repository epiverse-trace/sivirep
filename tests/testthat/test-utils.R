...
test_that("`obtener_meses_mas_casos` funciona correctamente", {
   ...
   result_meses <- obtener_meses_mas_casos(data_event = casos_inisintomas,
                                           col_fechas = "ini_sin",
                                           col_casos = "casos",
                                           top = 3,
                                           concat_vals = TRUE)
...
test_that("`obtener_meses_mas_casos` maneja errores correctamente", {
  ...
  expect_error(obtener_meses_mas_casos(data_event = list(a = 1, b = 2),
                                       col_fechas = "ini_sin"),
               "El parametro data_event debe ser un data.frame")
  
  expect_error(obtener_meses_mas_casos(data_event = data_limpia,
                                       col_fechas = 1),
...
test_that("`obtener_tip_ocurren_geo` funciona correctamente", {
  ...
  expect_equal(result_tip, c("cod_dpto_o", "departamento_ocurrencia",
...
test_that("`obtener_info_depts` funciona correctamente", {
  ...
  result_mpio <-  obtener_info_depts(dpto = "bogota dc",
                                     mpio = "bogota dc")
  expect_s3_class(result_mpio, "data.frame")
  expect_equal(nrow(result_mpio), 1)
})
...
test_that("`obtener_info_depts` maneja errores correctamente", {
  ...
  expect_error(obtener_info_depts(),
               "El parametro dpto es obligatorio")
  expect_error(obtener_info_depts(dpto = TRUE),
...
test_that("`obtener_nombre_dpto` maneja errores correctamente", {
  ...
  expect_error(obtener_nombre_dpto(list(a = 1, b = 2)),
               "El parametro data_geo debe ser un data.frame")
  expect_error(obtener_nombre_dpto(data.frame(cod_dpto = 05, dpto = "antioquia")),
...
test_that("`obtener_nombre_mpio` maneja errores correctamente", {
  ...
  expect_error(obtener_nombre_mpio(list(a = 1, b = 2)),
               "El parametro data_geo debe ser un data.frame")
  expect_error(obtener_nombre_mpio(data.frame(cod_dpto = 05, dpto = "antioquia")),
               "El parametro cod_dpto es obligatorio")
  expect_error(obtener_nombre_mpio(data.frame(cod_dpto = 05, dpto = "antioquia"),
                                   cod_dpto = 05),
...
test_that("`obtener_text_sex` funciona correctamente", {
  ...
  expect_equal(text_sex$text,
               paste0("En el total de casos para 2020 se ",
