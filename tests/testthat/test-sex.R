...
test_that("`sex` maneja errores correctamente", {
  ...
  expect_error(agrupar_sex(data_event = list(a = 1, b = 2)),
               "El parametro data_event debe ser un data.frame")
  
  expect_error(agrupar_sex(data_event = data_limpia, col_sex = TRUE),
               "El parametro col_sex debe ser una cadena de caracteres")
  
  expect_error(agrupar_sex(data_event = data_limpia,
                           porcentaje = "boolean"),
               "El parametro porcentaje debe ser un booleano")
  
  expect_error(plot_sex(data_agrupada = list(a = 1, b = 2)),
               "El parametro data_agrupada debe ser un data.frame")
  
  expect_error(plot_sex(data_agrupada = data_limpia,
                        col_sex = 1),
               "El parametro col_sex debe ser una cadena de caracteres")
})
...
test_that("`sex_semanaepi` maneja errores correctamente", {
  ...
  expect_error(agrupar_sex_semanaepi(data_event = list(a = 1, b = 2)),
               "El parametro data_event debe ser un data.frame")
  
  expect_error(agrupar_sex_semanaepi(data_event = data_limpia,
                                     cols_sex = TRUE),
  ...
  expect_error(plot_sex_semanaepi(data_agrupada = list(a = 1, b = 2)),
               "El parametro data_agrupada debe ser un data.frame")

  expect_error(plot_sex_semanaepi(data_agrupada = data_limpia,
                                  col_sex = 1),
               "El parametro col_sex debe ser una cadena de caracteres")
