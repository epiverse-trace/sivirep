...
test_that("`mapa` maneja errores correctamente", {
  ...
  expect_error(plot_map(data_agrupada = list(a = 1, b = 2)),
               "El parametro data_agrupada debe ser un data.frame")
  expect_error(plot_map(data_agrupada = data_limpia, fuente_data = 1),
...
test_that("`mapa_colombia` funciona correctamente", {
  ...
  map <- plot_map(data_agrupada = data_espacial,
...
test_that("`mapa_dpto` funciona correctamente", {
  ...
  map <- plot_map(data_agrupada = data_espacial_dpto,
...
test_that("`mapa_mpio` funciona correctamente", {
  ...
  map <- plot_map(data_agrupada = data_espacial_mpio,
                  col_distribucion = "casos",
                  dpto = "Antioquia",
                  mpio = "Envigado")
...
test_that("`mapa_indicidencia_colombia` funciona correctamente", {
  ...
  incidencia_dptos <- calcular_incidencia_geo(data_agrupada =
                                               data_agrupada)
...
test_that("`mapa_indicidencia_dpto` funciona correctamente", {
  ...
  data_agrupada <- agrupar_mpio(data_event = data_estandar,
                                dpto = "Antioquia")
...
test_that("`mapa_indicidencia_mpio` funciona correctamente", {
  ...
  data_filtrada_mpio <- geo_filtro(data_event = data_estandar,
                                   dpto = "Antioquia",
                                   mpio = "Medellin")
  data_agrupada <- agrupar_mpio(data_event = data_filtrada_mpio,
                                dpto = "Antioquia")
  incidencia_mpio <- calcular_incidencia_geo(data_agrupada =
                                                data_agrupada)
