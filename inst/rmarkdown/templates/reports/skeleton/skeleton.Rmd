...
params:
  disease_name:
    ...
    label: Enfermedad
    input: select
    choices: !r sivirep::list_available_diseases_years()$enfermedad
  year:
    value: 2020
    label: Año
  department:
    ...
    label: Departamento
    input: select
    choices: !r sivirep::get_departments()
  municipalitie:
    value: ""
    label: Municipio
---
...
```{r group_distribution_municipalities,  results='hide', echo = FALSE, error = FALSE, warning = FALSE, include = FALSE, message = FALSE}
...
Los casos se distribuyen a lo largo de `r nrow(spatial_muns_dist)` municipios del departamento de `r params$department`, teniendo un mayor reporte el municipio de `r stringr::str_to_title(spatial_most_cases$nombre)` con `r spatial_most_cases$casos` casos (Ver Figura 7).
