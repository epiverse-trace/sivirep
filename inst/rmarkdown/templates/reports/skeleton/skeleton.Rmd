...
```{r clean_data, include = FALSE}
filter_disease_data <-  geographical_filter(clean_disease_data, params$department)
```
...
# Distribución por municipio de los casos

```{r plot_distribution_municipalitie, fig.height = 5, fig.width = 10, fig.cap = "Distribucion de casos por municipio"}
...
spatial_muns_dist <- group_municipalities(disease_data = filter_disease_data)
map <- plot_map(data_grouped = spatial_muns_dist, department = params$department)
```
