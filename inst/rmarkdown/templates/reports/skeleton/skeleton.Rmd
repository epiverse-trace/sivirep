...
# TODO Poner aquí todas las variables que correspondan a texto que se muestre en diferentes
# partes del reporte

titulo <- paste0(
  "Reporte Automatizado para ",
...
# TODO Mover esto a una celda exclusivamente para inicializar datos
# Esta celda debe ir antes que la celda que inicializa las variables de textos
data_event <- data.frame()
withCallingHandlers(
  {
    data_event <- import_data_event(
      nombre_event = params$nombre_evento,
      years = params$year,
      cache = params$cache,
      ruta_dir = ruta_dir
    )
...
# TODO Mover a celda de textos
mostrar_sex <- stringr::str_detect(
  tolower(params$nombre_evento),
...
# TODO Mover a celda de inicialización de datos y poner en una función
...
# TODO Todas estas variables de texto juntarlas en el mismo lugar de e
  text_init_geo <- paste0(
...
total_casos_eventos <- agrupar_eventos(
  data_event = data_event_filtrada,
  col_event = "cod_eve"
)
...
# TODO Mover a una función, mover al bloque de inicialización de datos
...
data_event_years <- rbind(data_event[, cols], data_event_years[, cols])
data_years_limpia <- limpiar_data_sivigila(data_event_years)
data_years_filtrada <- data_years_limpia
if (params$departamento != "") {
  data_years_filtrada <- geo_filtro(
    data_event = data_years_limpia,
    dpto = params$departamento
  )
}
#############################
# TODO Mover a una función (incluir siguiente bloque)
...
# TODO Mover a una función (incluir siguiente bloque)
casos_tip_cas_years <- agrupar_tipo_caso(
  data_event = data_years_filtrada,
...
# TODO Mover a una función (incluir siguiente bloque)
casos_iniciosin_dia <- agrupar_fecha_inisintomas(
  data_event =
    data_event_filtrada
)
mes_mayor_casos <- obtener_meses_mas_casos(
  data_event =
    casos_iniciosin_dia,
  col_fechas = "ini_sin"
)
...
plot_fecha_inisintomas(
  data_agrupada = casos_iniciosin_dia,
  uni_marca = "semanaepi"
)
```
...
# TODO Mover a sección de inicialización de datos
...
espacial_mayor_casos <- obtener_fila_mas_casos(
  data_event =
    dist_espacial
)
altura_fig_espacial <- 15
if (isTRUE(length(unique(
  dist_espacial[[col_geo]]
)) < 15)) {
...
mapa <- plot_map(
  data_agrupada = dist_espacial_casos,
...
# TODO Mover a sección de inicialización de datos. Incluir siguiente celda
...
incidencia_total <- calcular_incidencia(
  data_agrupada =
    dist_espacial,
  cache = params$cache,
  ruta_dir = ruta_dir
...
# TODO Mover a seccion inicialización de datos
incidencias_sex <-
...
incidencia_mayor_sex <- obtener_fila_mas_casos(
  data_event =
...
plot_sex(
  data_agrupada = incidencias_sex$data_incidencia,
...
# TODO Mover a una función, incluir siguiente celda
if (params$departamento == "" && params$municipio == "") {
...
mapa <- plot_map(
  data_agrupada = incidencias_geo$data_incidencia,
  cache = params$cache,
  ruta_dir = ruta_dir
)
```
...
text_casos_totales <- paste0(
  "*   Los casos totales reportados de ",
  stringr::str_to_title(params$nombre_evento),
