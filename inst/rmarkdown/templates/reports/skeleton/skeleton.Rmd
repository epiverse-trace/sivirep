...
```{r plot_cases_by_tip_cas, fig.height = 5, fig.width = 11, fig.cap = paste0("Distribución de casos por clasificación - ", text_ext_geo)}
...
```{r plot_tip_cas_by_years, fig.height = 7, fig.width = 13, fig.cap = paste0("Distribución de casos en los últimos 6 años - ", text_ext_geo)}
...
`r if (params$temporal_distribution) {paste0("Los casos se distribuyen a lo largo de los meses de enero a diciembre de ", params$year, " teniendo un mayor reporte en el mes de ", mes_mayor_casos, " por fecha de inicio de síntomas (Ver Figura 4).")}`

\newpage

```{r plot_cases_by_onset_symptom_date, fig.height = 5, fig.width = 11, fig.cap = paste0("Distribución de casos por fecha de inicio de sintomas - ", text_ext_geo), include = params$temporal_distribution, eval = params$temporal_distribution}
...
```{r distribution_cg_plots, fig.height = 4.8, fig.width = 8.4, fig.cap = paste0("Distribución de casos por sexo - ", text_ext_geo), include = params$epi_sex_distribution}
...
`r if (params$epi_sex_distribution) {paste0("Esta predominancia del reporte de casos del sexo ", vals_sex$mayor[1], " se mantuvo a lo largo de la mayoria de semanas epidemiológicas (Ver figura 6). ")}`

```{r plot_cg_semanaepi, fig.height = 6, fig.width = 10, fig.cap =  paste0("Distribución de casos por sexo y semana epidemiológica - ", text_ext_geo), include = params$epi_sex_distribution, fig.pos = "H"}
...
```{r distribution_age, fig.height = 3, fig.width = 8, fig.cap =  paste0("Distribución de casos por edad - ", text_ext_geo), include = params$age_distribution}
...
```{r distribution_age_anger, fig.height = 3, fig.width = 9, fig.cap =  paste0("Distribución de casos por edad y sexo - ", text_ext_geo), include = params$age_sex_distribution && !mostrar_sex}
casos_edad_sex <- agrupar_edad_sex(data_event = data_event_filtrada,
                                   interval_edad = 10)
plot_edad_sex(data_agrupada = casos_edad_sex)
```

`r if(params$mpio_distribution && params$departmento != "") {"# Distribución de casos por municipio"} else {"# Distribución de casos por departamento"}`
...
if (params$departmento != "") {
  ...
  dist_espacial_dpto <- agrupar_dpto(data_event = data_event_filtrada)
...
if (isTRUE(length(unique(
  dist_espacial_dpto[[col_geo]])) < 15)) {
  altura_fig_espacial <- 10
}
```

`r if(params$mpio_distribution && params$departmento != "") {paste0("Los casos se distribuyen a lo largo de ", nrow(dist_espacial_dpto), " municipios del departamento de ", params$departmento, " teniendo un mayor reporte el municipio de ", stringr::str_to_title(espacial_mayor_casos[[col_geo]]), " con ", espacial_mayor_casos$casos, " casos (Ver Figura 9).")} else {paste0("Los casos se distribuyen a lo largo de ", nrow(dist_espacial_dpto), " departamentos de ", params$pais, " teniendo un mayor reporte el departamento de ", stringr::str_to_title(espacial_mayor_casos[[col_geo]]), " con ", espacial_mayor_casos$casos, " casos (Ver Figura 9).")}`

```{r plot_distribution_municipios, fig.height = altura_fig_espacial, fig.width = 10, fig.cap = paste0("Distribución de casos por ", text_fig_geo), include = params$mpio_distribution && params$departmento != "", eval = params$mpio_distribution && params$departmento != ""}
plot_mpios <- plot_mpios(data_agrupada = dist_espacial_dpto)
plot_mpios
```

```{r plot_distribution_dptos, fig.height = altura_fig_espacial, fig.width = 10, fig.cap = paste0("Distribución de casos por ", text_fig_geo), eval = params$departmento == "", include = params$departmento == ""}
plot_dptos <- plot_dptos(data_agrupada = dist_espacial_dpto)
plot_dptos
```
...
```{r plot_distribution_areas, fig.height = 3, fig.width = 7, fig.cap = paste0("Distribución de casos por área geográfica - ", text_ext_geo), include = params$areas_distribution, fig.pos = "H"}
...
Los 10 `r etiqueta_geo` que presentaron mayor número de casos por área geográfica son los siguientes:

```{r plot_top_areas, fig.height = 5, fig.width = 10, fig.cap = paste0("Distribución de los 10 municipios con más casos por área geográfica - ", text_ext_geo), include = params$areas_distribution, fig.pos = "H"}
...
```{r distribution_per_etn, fig.height = 3, fig.width = 8, fig.cap = paste0("Distribución de casos por pertenencia étnica - ", text_ext_geo), include = params$age_sex_distribution}
...
`r if(params$mpio_distribution && params$departmento != "") {paste0("La incidencia para el departamento de ", params$departmento, " es ", incidencia_dpto, " por ", cond_incidencia$coeficiente, " habitantes.")} else {paste0("La incidencia para ", params$pais, " es ", incidencia_dpto, " por ", cond_incidencia$coeficiente, " habitantes.")}`
...
```{r sex_incidence_plot, fig.height = 3, fig.width = 7, fig.cap = paste0("Incidencia por sexo - ", text_ext_geo), include = params$epi_sex_distribution}
...
`r if(params$spatial_distribution && params$departmento != "") {paste0("La incidencia para cada uno de los municipios del departamento del ", params$departmento, " por ", cond_incidencia$coeficiente, " habitantes es la siguiente:")} else {paste0("La incidencia para cada uno de los departamentos de ", params$pais, " por ", cond_incidencia$coeficiente, " habitantes es la siguiente:")}`
...
```{r map_distribution_spatial, echo = FALSE, error = FALSE, warning = FALSE, include = TRUE, message = FALSE, cache = FALSE, results = FALSE, comment = FALSE, fig.height = 18, fig.width = 16, fig.cap = paste0("Incidencia según distribución geográfica - ", text_ext_geo), include = params$spatial_distribution}
mapa
```
...
if (params$spatial_distribution && params$departmento != "") {
  text_geo <- paste0("el departamento de ", params$departmento)
}
...
`r if(params$spatial_distribution && params$departmento != "") text_casos_totales`
...
* La incidencia para `r text_geo` es de `r incidencia_dpto` por  `r cond_incidencia$coeficiente` habitantes.
