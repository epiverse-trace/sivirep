...
La distribución de los casos por grupos etarios muestra una tendencia decreciente a medida que se avanza en la edad. La población de `r age_most_cases$edad` años representó el `r  age_most_cases$porcentaje` % de todos los casos de `r stringr::str_to_title(params$nombre_evento)` en el país (Ver figura 5). De forma interesante, aunque el sexo masculino predominó en casi todos los grupos etários, para el grupo de 0-10 años esta predominancia fue mucho menos marcada (Ver Figura 6).
...
b.  Los meses de `r meses_mayor_casos` tuvieron el mayor reporte.
c.  El reporte de casos de `r stringr::str_to_title(params$nombre_evento)` a lo largo de `r params$year` predominó en la población de `r age_most_cases$edad` años.
d.  El reporte de casos de `rstringr::str_to_title(params$nombre_evento)` predominó a lo largo del año en el sexo masculino.
