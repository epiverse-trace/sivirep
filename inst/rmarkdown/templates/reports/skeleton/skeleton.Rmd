...
knitr::opts_chunk$set(include = TRUE, echo = FALSE, error = FALSE,  warning = FALSE, message = FALSE)
library(sivirep)
```

---
title: "Reporte Automatizado para `r params$disease_name`, Colombia `r params$year`"
---
...
clean_disease_data <-  clean_sivigila_data(disease_data, params$year)
```
...
onset_symptoms_data <- group_onset_symptoms(clean_disease_data, type = "month") 
months_major_cases <- get_months_most_cases(onset_symptoms_data, col_dates = "ini_sin")
```

Los casos se distribuyen a lo largo de los meses de enero a diciembre de `r params$year`, teniendo un mayor reporte en los meses `r months_major_cases` tanto por fecha de inicio de síntomas (Ver Figura 1) como por fecha de notificación (Ver Figura 2). Este comportamiento pudo haber sido distinto a años previos debido al inicio de estrategias de control de COVID-19 a comienzos de marzo de `r params$year`.
...
notification_date_data <- group_notification_date(clean_disease_data, type = "month")
```
...
gender_data <- group_gender(clean_disease_data)
male_percentage <- gender_data$porcentaje[2]
female_percentage <- gender_data$porcentaje[1]
gender_most_cases <- get_most_cases(gender_data)
```
...
```{r distribution_cg_plot, fig.height = 3, fig.width = 7, fig.cap =  "Distribucion de casos por genero"}
...
gender_week_data <- group_gender_epiweek(clean_disease_data)
```
...
plot_gender_epiweek(gender_week_data)
```

# Distribución de casos por edad

```{r distribution_age_d, include = FALSE}
age_week_data <- group_age_epiweek(clean_disease_data)
age_range_most_cases <- get_most_cases(age_week_data)
```

La distribución de los casos por grupos etarios muestra una tendencia decreciente a medida que se avanza en la edad. La población de `r age_range_most_cases$edad` años representó el `r  age_range_most_cases$porcentaje` % de todos los casos de `r params$disease_name` en el país (Ver figura 5). De forma interesante, aunque el sexo masculino predominó en casi todos los grupos etários, para el grupo de 0-10 años esta predominancia fue mucho menos marcada (Ver Figura 6).

```{r distribution_age, fig.height = 3, fig.width = 8, fig.cap =  "Distribucion de casos por edad"}
plot_age_epiweek(age_week_data)
```
...
age_gender_data <- group_age_gender(clean_disease_data)
plot_age_gender(age_gender_data)
```
...
special_population_data <- group_special_population(clean_disease_data)
...
departments_spacial_data <- group_deptos(clean_disease_data)
...
b.  Los meses de `r months_major_cases` tuvieron el mayor reporte. Es posible que haya un impacto en el menor reporte y/o menor transmisión de `r params$disease_name` como producto de la pandemia COVID-19 y medidas de control que iniciaron en marzo de `r params$year`.
c.  El reporte de casos de `r params$disease_name` a lo largo de `r params$year` predominó en la población de `r age_range_most_cases$edad` años.
