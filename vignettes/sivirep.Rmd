...
```{r} 
geo_data_codes <- import_geo_codes()
```
...
```{r} 
depto_disease_codes_data <- clean_depto_disease_codes(geo_data_codes, disease_data, make_group = TRUE)
```
...
```{r} 
disease_data_by_week <- group_by_week_and_cases()
```
...
```{r}
disease_data_by_age <- group_by_age_range_and_cases(disease_data, min_val = 5, max_val = 82, step = 5)
```
...
```{r}
filtered_data <- filter_disease(name_disease = "MALAR", sivigila_data)
...
```{r}
map <- plot_dept_map(depto_disease_codes_data, col_name_lj = "id")
map
```
