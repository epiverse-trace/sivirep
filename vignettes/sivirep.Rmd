...
depto_codes <- clean_depto_disease_codes(depto_codes, disease_data, make_group = TRUE)
```
...
```{r}
disease_data_by_age <- group_by_age_range_and_cases(disease_data_by_age_and_week, min_val = 5, max_val = 82, step = 5)
```
...
```{r}
plot_epiweek(disease_data_by_week, col_week = "SEMANA", col_cases = "Casos", year = 2019, type = "date", xlabel = "Fecha de semana epidemiologica")
```
