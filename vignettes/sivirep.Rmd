...
pak::pak("epiverse-trace/sivirep") # nolint: nonportable_path_linter
```
...
```{r import_data_event}
data_event <-  import_data_event(nombre_event = "Dengue",
                                 year = 2020)
```
...
El gráfico que permite visualizar esta distribución debe generarse con la función `plot_fecha_notifica`. Ten en cuenta que, aunque hayas agrupado los datos por día, es posible que prefieras representarlos por semana epidemiológica, como en:
