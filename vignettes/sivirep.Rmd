...
1. In RStudio click *File/New File/R* Markdown and the following window will appear:

2. Select the left panel option: From Template, then select the template report called **"Reporte Básico {sivirep}”"**, and indicate the name you want for the report (i.e. MyReport_Laura) and the location to store it.

3. Then, you can select the disease (the disease name) and the year of the report. This action will download the data wanted and also will provide the template in R Markdown .Rmd file. For this, it is important to find Knit buttom, display the options and select ‘Knit with parameters’.

4. Wait a few seconds while the report is rendered. You can add, edit, delete and customise sections of the report in the R Markdown file generated earlier. 
...
```{r}
onset_symptoms_by_day <- group_onset_symptoms(clean_disease_data, type = "day") 
onset_symptoms_by_month <- group_onset_symptoms(clean_disease_data, type = "month") 
```
...
```{r}
plot_onset_symptoms(data_grouped = onset_symptoms_by_day,
...
```{r}
notification_date_by_day <- group_notification_date(clean_disease_data, 
                                                    type = "day") 
notification_date_by_month <- group_notification_date(clean_disease_data, 
                                                      type = "month") 
```
...
```{r}
plot_notification_date(data_grouped = notification_date_by_day,
                       break_tick_date = "months")
```
