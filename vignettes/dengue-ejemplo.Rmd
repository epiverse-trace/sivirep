...
En total en Colombia durante 2020 se reportaron `r nrow(disease_data)` casos en todo el territorio nacional. 
...
La distribución de los casos por grupos etarios muestra una tendencia decreciente a medida que se avanza en la edad. La población menor de 10 años representó el   `r round((first(disease_dt_by_age_range$Casos)[1]/nrow(disease_data) * 100), 2) ` % de todos los casos de Dengue en el país (Ver figura 5).
...
a. Los casos totales reportados de Dengue en Colombia en el año 2020 fueron `r nrow(disease_data)`.
