...
- `sivirep` 0.0.1 is meant to assist with access to SIVIGILA source. This process of downloading the information may take a few minutes depending on the size of the dataset. To avoid repeating downloading the same data every time you can use `cache = TRUE` in the function `import_linelist_disease_year` This option is set by default.
...
- When building a report section or analysing this data it can be useful to get the months with the most cases, in `sivirep` you can use the function `get_months_most_cases` to get this information.
...
- When building a report section or analysing this data it can be helpful to know which gender has the most cases, in `sivirep` you can use the function `get_most_cases` to get this information. This function works with any dataset that contains a column called “cases” by any level of aggregation.